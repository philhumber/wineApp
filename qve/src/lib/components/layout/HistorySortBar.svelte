<script lang="ts">
  ...
  function handleSortChange(event: Event) {
    ...
    // Text fields (name, type) default to ascending (A-Z)
    // Numeric fields (date, ratings) default to descending (newest/highest first)
    const defaultDir = key === 'wineName' || key === 'wineType' ? 'asc' : 'desc';
