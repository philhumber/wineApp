...
class EnrichmentService
{
    ...
    // ===========================================
    // Phase 6: Streaming Support Methods
    // ===========================================
    ...
    public function checkCache(array $identification, bool $confirmMatch): ?EnrichmentResult
    {
        ...
        // Non-exact match needs confirmation
        if ($matchType !== 'exact' && !$confirmMatch) {
            return $this->buildPendingConfirmation($cacheResult, $identification);
        }
     ...
     * Validates, caches, and merges with fallback data.
     *
     * @param EnrichmentData $data Raw enrichment data from LLM
     * @param array $identification Parsed identification data
     * @param bool $forceRefresh Whether to skip cache merge
     * @return EnrichmentResult
     */
    public function processEnrichmentResult(
        ...
        $warnings = $this->validator->validate($data, $vintage, $wineType);
        $result->warnings = $warnings;

        // Cache if confidence meets threshold
        if ($data->confidence >= $thresholds['store_minimum']) {
            $this->cache->set($producer ?? '', $wineName ?? '', $vintage, $data);
        }
        ...
        if ($data->confidence < $thresholds['merge_threshold']) {
            $inference = $this->fallback->inferFromReference($producer, $wineName, $region, $wineType);
        }

        $mergeResult = $this->merger->merge($data, $cached, $inference);
    ...
    /**
     * Stream enrichment from LLM via WebSearchEnricher.
     * Delegates to the enricher's streaming method.
     *
     * @param array $identification Parsed identification data
     * @param callable $onField fn(string $field, mixed $value)
     * @return EnrichmentData|null
     */
    public function enrichStreaming(array $identification, callable $onField): ?EnrichmentData
    {
        $producer = $identification['parsed']['producer'] ?? '';
        $wineName = $identification['parsed']['wineName'] ?? '';
        $vintage = $identification['parsed']['vintage'] ?? null;

        return $this->enricher->enrichStreaming($producer, $wineName, $vintage, $onField);
    }
}
